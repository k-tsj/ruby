...
iseq_compile_pattern_each(rb_iseq_t *iseq, LINK_ANCHOR *const cond_seq, const NODE *const node, int in_alt_pattern)
{
    switch (nd_type(node)) {
      case NODE_ADECON: {
        const NODE *vals = node;
        int line = nd_line(node);
        struct rb_ary_deconstructor_info *adinfo = vals->nd_adinfo;
        const NODE *args = adinfo->pre_args_node;
        const int pre_args_num = adinfo->pre_args_node ? adinfo->pre_args_node->nd_alen : 0;
        const int post_args_num = adinfo->post_args_node ? adinfo->post_args_node->nd_alen : 0;

        const int min_argc = pre_args_num + post_args_num;
        const int max_argc = adinfo->rest_arg_node ? UNLIMITED_ARGUMENTS : min_argc;
        LABEL *next_cond, *fin;
        int i, rest_start;
        next_cond = NEW_LABEL(line);
        fin = NEW_LABEL(line);

        if (adinfo->const_node) {
            DECL_ANCHOR(head);
            INIT_ANCHOR(head);
            ADD_INSN(cond_seq, nd_line(vals), dup);
            ADD_INSN(cond_seq, nd_line(vals), dup);
            CHECK(COMPILE(head, "constant", adinfo->const_node));
            ADD_SEQ(cond_seq, head);
            ADD_INSN1(cond_seq, nd_line(vals), checkmatch, INT2FIX(VM_CHECKMATCH_TYPE_CASE));
        ...
        if (adinfo->rest_arg_node) {
            ...
            ADD_INSN(cond_seq, nd_line(vals), dup);
            ADD_CALL(cond_seq, nd_line(vals), idLength, INT2FIX(0));
            ADD_INSN1(cond_seq, nd_line(vals), putobject, INT2FIX(pre_args_num + post_args_num));
            ...
            iseq_compile_pattern_each(iseq, cond_seq, adinfo->rest_arg_node, in_alt_pattern);
            ADD_INSNL(cond_seq, nd_line(vals), branchunless, next_cond);
        }

        rest_start = pre_args_num;
        args = adinfo->post_args_node;
        for (i = 0; i < post_args_num; i++) {
            ...
            ADD_INSN1(cond_seq, nd_line(vals), putobject, INT2FIX(rest_start + i));
      ...
      case NODE_HDECON: {
        const NODE *vals = node;
        int line = nd_line(node);
        struct rb_hash_deconstructor_info *hdinfo = vals->nd_hdinfo;
        LABEL *next_cond, *fin;
        VALUE keys = Qnil;
        ...
        if (hdinfo->kw_args_node && !hdinfo->kw_rest_arg_node) {
            const NODE *kw_args = hdinfo->kw_args_node->nd_head;
        ...
        if (hdinfo->const_node) {
            DECL_ANCHOR(head);
            INIT_ANCHOR(head);
            ADD_INSN(cond_seq, nd_line(vals), dup);
            ADD_INSN(cond_seq, nd_line(vals), dup);
            CHECK(COMPILE(head, "constant", hdinfo->const_node));
            ADD_SEQ(cond_seq, head);
            ADD_INSN1(cond_seq, nd_line(vals), checkmatch, INT2FIX(VM_CHECKMATCH_TYPE_CASE));
            ADD_INSNL(cond_seq, nd_line(vals), branchunless, next_cond);
            ADD_INSN(cond_seq, nd_line(vals), pop);
        ...
        ADD_INSN(cond_seq, nd_line(vals), dup);
        ADD_INSN(cond_seq, nd_line(vals), dup);
        ADD_INSN1(cond_seq, nd_line(vals), putobject, ID2SYM(rb_intern("deconstruct_keys")));
        ADD_CALL(cond_seq, nd_line(vals), idRespond_to, INT2FIX(1));
        ADD_INSNL(cond_seq, nd_line(vals), branchunless, next_cond);

        ADD_INSN1(cond_seq, nd_line(vals), putobject, keys);
        ADD_CALL(cond_seq, nd_line(vals), rb_intern("dup"), INT2FIX(0));

        ADD_CALL(cond_seq, nd_line(vals), rb_intern("deconstruct_keys"), INT2FIX(1));
        ...
        if (hdinfo->kw_args_node) {
            int i;
            int keys_num;
            const NODE *args;
            args = hdinfo->kw_args_node->nd_head;
            if (args) {
        ...
        if (hdinfo->kw_rest_arg_node) {
            ADD_INSN(cond_seq, nd_line(vals), dup);
            iseq_compile_pattern_each(iseq, cond_seq, hdinfo->kw_rest_arg_node, in_alt_pattern);
            ADD_INSNL(cond_seq, nd_line(vals), branchunless, next_cond);
        }
	...
	ADD_SETLOCAL(cond_seq, nd_line(node), idx, get_lvar_level(iseq));
...
compile_case3(rb_iseq_t *iseq, LINK_ANCHOR *const ret, const NODE *const orig_node, int popped)
{
    ...
    if (type != NODE_IN) {
	COMPILE_ERROR(ERROR_ARGS "NODE_CASE3("TOSTR(__LINE__)"): unexpected node. must be NODE_IN, but %s", ruby_node_name(type));
	return COMPILE_NG;
    }
	...
	if (vals) {
	    ...
	    EXPECT_NODE_NONULL("NODE_CASE3("TOSTR(__LINE__)")", node, NODE_ARRAY, COMPILE_NG);
	}
