...
rb_vm_pop_cfunc_frame(void)
{
    ...
    EXEC_EVENT_HOOK(th, RUBY_EVENT_C_RETURN, cfp->self, me->called_id, me->called_id, me->owner, Qnil); /* TODO */
...
invoke_bmethod(rb_thread_t *th, const rb_iseq_t *iseq, VALUE self, const struct rb_captured_block *captured, const rb_callable_method_entry_t *me, VALUE type, int opt_pc)
{
    ...
    EXEC_EVENT_HOOK(th, RUBY_EVENT_CALL, self, me->called_id, me->called_id, me->owner, Qnil); /* TODO */
    ret = vm_exec(th);
    EXEC_EVENT_HOOK(th, RUBY_EVENT_RETURN, self, me->called_id, me->called_id, me->owner, ret); /* TODO */
...
hook_before_rewind(rb_thread_t *th, rb_control_frame_t *cfp, int will_finish_vm_exec)
{
	...
	if (VM_FRAME_BMETHOD_P(th->cfp)) {
		...
		EXEC_EVENT_HOOK_AND_POP_FRAME(th, RUBY_EVENT_RETURN, th->cfp->self,
					      ...
					      rb_vm_frame_method_entry(th->cfp)->called_id, /* TODO */
...
int
rb_vm_control_frame_id_and_class(const rb_control_frame_t *cfp, ID *idp, ID *callee_idp, VALUE *klassp)
{
    const rb_callable_method_entry_t *me = rb_vm_frame_method_entry(cfp);

    if (me) {
	if (idp) *idp = me->def->original_id;
	if (callee_idp) *callee_idp = me->called_id;
...
int
rb_frame_method_id_and_class(ID *idp, ID *callee_idp, VALUE *klassp)
{
    return rb_thread_method_id_and_class(GET_THREAD(), idp, callee_idp, klassp);
}
