...
call_trace_func(rb_event_flag_t event, VALUE proc, VALUE self, ID id, VALUE klass)
{
    ...
    if (!klass) {
	rb_thread_method_id_and_class(th, &id, 0, &klass); /* TOOD */
    }
...
fill_id_and_klass(rb_trace_arg_t *trace_arg)
{
	...
	if (!trace_arg->klass) {
	    rb_vm_control_frame_id_and_class(trace_arg->cfp, &trace_arg->id, &trace_arg->callee_id, &trace_arg->klass);
	}
...
{
    fill_id_and_klass(trace_arg);
    return trace_arg->callee_id ? ID2SYM(trace_arg->callee_id) : Qnil;
}
